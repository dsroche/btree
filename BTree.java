public class BTree<K extends Comparable<? super K>, V> extends java.util.AbstractMap<K,V> {
  ...
  public static final int LeafM = 3;
  /** Maximum number of key/child pairs in an internal node. */
  public static final int InternalM = 3;
    ...
    extends Node<K,V,Node<K,V,?>>
  {
    ...
    @Override
    public void printMe(String indent) {
      this.leftEntry(this.get(0).getKey()).leaf.printMe(indent+"     ");
      for (int i = 0; i < this.size(); i++) {
	System.out.println(indent + this.get(i).getKey());
  ...
  private void printSubtree (Node<K,V,?> node, String indent) {
    if (node instanceof LeafNode) {
      for (int i = 0; i < node.size(); i++)
	       System.out.println(indent + node.get(i).getKey());
    } else {
      printSubtree(node.leftEntry(node.get(0).getKey()).leaf, indent+"     ");
      for (int i = 0; i < node.size(); i++) {
        System.out.println(indent + node.get(i).getKey());
        printSubtree((Node<K,V,?>)node.get(i).getValue(), indent+"     ");
      }
    }
  }
  ...
  public class Iter implements java.util.Iterator<Entry<K,V>> {
    ...
    /** Creates a new instance attached to the containing BTree class. */
    public Iter() {
      if (isEmpty()) return;
